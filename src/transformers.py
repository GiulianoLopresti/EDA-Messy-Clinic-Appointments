...
class BillingCleanerTransformer(BaseEstimator, TransformerMixin):
    ...
    Técnica usada:
        ...
        r'[\d.]+' significa "uno o más caracteres que sean dígito o punto".
    """
    def fit(self, X, y=None):
        return self
...
class DateFeatureTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if appt_col in X_copy.columns and booking_col in X_copy.columns:
            ...
            appt_dates    = pd.to_datetime(X_copy[appt_col],    infer_datetime_format=True, errors="coerce")
            booking_dates = pd.to_datetime(X_copy[booking_col], infer_datetime_format=True, errors="coerce")
